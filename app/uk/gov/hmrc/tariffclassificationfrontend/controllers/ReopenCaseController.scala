...
class ReopenCaseController @Inject()(verify: RequestActions,
  ...
  def confirmReopenCase(reference: String): Action[AnyContent] = (verify.authenticate andThen verify.casePermissions(reference) andThen verify.mustHave(Permission.OPEN_CASE)).async { implicit request =>
