...
class RejectCaseController @Inject()(actions: AuthenticatedControllerActions,
  ...
  def rejectCase(reference: String): Action[AnyContent] = actions.authenticated.async { implicit request =>
    getCaseAndRenderView(reference, c => successful(views.html.reject_case(c, form)))
  }

  def confirmRejectCase(reference: String): Action[AnyContent] = actions.authenticated.async { implicit request =>
