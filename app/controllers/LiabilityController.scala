...
class LiabilityController @Inject() (
  ...
  def liabilityDetails(reference: String): Action[AnyContent] =
  ...
  def editLiabilityDetails(reference: String): Action[AnyContent] =
    (verify.authenticated andThen
      ...
      successful(
        Ok(
          views.html.partials.liabilities.liability_details_edit(
            request.`case`,
  ...
  def postLiabilityDetails(reference: String): Action[AnyContent] =
    (verify.authenticated andThen
      ...
      LiabilityDetailsForm
        ...
        .fold(
          errorForm =>
            successful(Ok(views.html.partials.liabilities.liability_details_edit(request.`case`, errorForm))),
          updatedCase =>
            getCaseAndRedirect(
              menuTitle,
              c =>
  ...
  private def getCaseAndRenderView(page: CaseDetailPage, toHtml: Case => Future[HtmlFormat.Appendable])(
    ...
    toHtml(request.`case`).map(html =>
      Ok(views.html.case_details(request.`case`, page, html, activeTab = Some(ActiveTab.Liability)))
    )
