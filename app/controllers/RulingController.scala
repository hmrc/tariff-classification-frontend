          ...
          c.application.`type` match {
            case ApplicationType.ATAR =>
              decisionForm.btiForm.bindFromRequest.fold(
                ...
                validForm =>
                  for {
                    update <- casesService.updateCase(mapper.mergeFormIntoCase(c, validForm), request.operator)
                  } yield Redirect(
            ...
            case ApplicationType.LIABILITY =>
              ...
              decisionForm
                ...
                .fold(
                  ...
                  updatedDecision =>
                    for {
                      update <- casesService.updateCase(c.copy(decision = Some(updatedDecision)), request.operator)
                    } yield Redirect(
