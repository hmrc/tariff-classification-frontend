...
object ManageKeywordsViewModel {
  def forManagedTeams(caseKeywords: Paged[CaseKeyword], allKeywords: Seq[Keyword]): ManageKeywordsViewModel = {

    val approvedKeywords = allKeywords.filter(_.approved)

    val keywordViewModel = caseKeywords.map(caseKeyword =>
      val caseHeader = caseKeyword.caseObj
    ...
    ManageKeywordsViewModel(
      "Manage keywords",
      ManageKeywordsTab("keywordsApproval", "approval_tab", keywordViewModel.filter(_.isNotApproved)),
