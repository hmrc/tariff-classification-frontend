...
class DatedCacheMapSpec extends SpecBase {
  "DatedCacheMap" should {
    "handle round-trip serialization/deserialization" when {
      "all the values are present" in {
        val original = new DatedCacheMap(
          id = "sdd",
          data = Map(("asd", JsString("sad"))),
          lastUpdated = Instant.parse("2021-01-01T09:00:00.00Z")
        )
        val json         = Json.toJson(original)
        val deserialized = json.as[DatedCacheMap]
    ...
    "fail to deserialize" when {
      "maxFileSize is not present" in {
        val json = Json.obj()
        json.validate[DatedCacheMap] shouldBe a[JsError]
      }
