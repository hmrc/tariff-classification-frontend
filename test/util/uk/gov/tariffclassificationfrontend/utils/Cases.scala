...
object Cases {
  ...
  val storedAttachment = StoredAttachment("id", application = true, public = true, None, Some("url"), "name", "type", Some(ScanStatus.READY), ZonedDateTime.now())
  val letterOfAuthority = StoredAttachment("id", application = true, public = true, None, Some("url"), "letterOfAuthority", "pdf", Some(ScanStatus.READY), ZonedDateTime.now())
  val eoriDetailsExample = EORIDetails("eori", "trader-business-name", "line1", "line2", "line3", "postcode", "country")
  val eoriAgentDetailsExample = AgentDetails(EORIDetails("eori", "agent-business-name", "line1", "line2", "line3", "postcode", "country"), Some(Attachment(UUID.randomUUID().toString, false, true, None, ZonedDateTime.now())))
