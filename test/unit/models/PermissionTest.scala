...
class PermissionTest extends ModelsBaseSpec {
  ...
  "Permissions" should {
    ...
    "contain 'Cancel a Case'" in {
      ...
      val caseWithValidBTIDecision = aCase(
        withStatus(CaseStatus.COMPLETED),
        withBTIApplication,
        withDecision(effectiveEndDate = Some(Instant.now().plusSeconds(10)))
      )
      ...
      val caseWithExpiredBTIDecision = aCase(
        withStatus(CaseStatus.COMPLETED),
        withBTIApplication,
        withDecision(effectiveEndDate = Some(Instant.now().minusSeconds(10)))
      )
    ...
    "contain 'Edit Extended Use Status'" in {
      ...
      for (status: CaseStatus <- CaseStatus.values.filterNot(equalTo(CaseStatus.CANCELLED))) {
        val caseWithInvalidStatus =
          aCase(withStatus(status), withDecision(cancellation = Some(Cancellation(CancelReason.ANNULLED))))
        permission.appliesTo(caseWithInvalidStatus, readOnly)   shouldBe false
        permission.appliesTo(caseWithInvalidStatus, teamMember) shouldBe false
        permission.appliesTo(caseWithInvalidStatus, manager)    shouldBe false
      }
