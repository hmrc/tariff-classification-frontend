...
class RejectCaseControllerSpec extends WordSpec with Matchers with UnitSpec
  ...
  "Reject Case" should {
    ...
    "return OK and HTML content type" in {
      ...
      val result: Result = await(controller.rejectCase("reference")(newFakeGETRequestWithCSRF(app)))
  ...
  "Confirm Reject a Case" should {
    ...
    "return OK and HTML content type" in {
      when(casesService.getOne(refEq("reference"))(any[HeaderCarrier])).thenReturn(successful(Some(caseWithStatusOPEN)))
      when(casesService.rejectCase(refEq(caseWithStatusOPEN), refEq(operator))(any[HeaderCarrier])).thenReturn(successful(caseWithStatusREFERRED))

      val result: Result = await(controller.confirmRejectCase("reference")(newFakePOSTRequestWithCSRF(app)))
    ...
    "propagate the error in case the CaseService fails to release the case" in {
      ...
      val caught = intercept[error.type] {
        await(controller.confirmRejectCase("reference")(newFakePOSTRequestWithCSRF(app)))
      }
