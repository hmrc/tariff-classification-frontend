...
class OpenCasesViewSpec extends ViewSpec {
  ...
  "OpenCasesViewSpec" should {
    ...
    "render successfully with the default tab" in {
      val doc = view(commonCasesView(CasesTabViewModel.atarCases(Seq(Cases.btiCaseExample, Cases.btiCaseExample))))

      doc should containElementWithID("open-cases-tabs")
    }
  }

  "contain an act, cap, cars and elm tabs for ATaR" in {
    val doc = view(commonCasesView(CasesTabViewModel.atarCases(Seq(Cases.btiCaseExample, Cases.btiCaseExample))))
  ...
  "contain an act, cap, cars and elm tabs for Liability" in {
    val doc = view(
      commonCasesView(CasesTabViewModel.liabilityCases(Seq(Cases.liabilityCaseExample, Cases.liabilityCaseExample)))
    )
  ...
  "contain an act, cap, cars and elm, flex ,tta,ttb,ttc tabs for Correspondence" in {
    val doc =
      view(commonCasesView(CasesTabViewModel.correspondenceCases(Seq(Cases.corrCaseExample, Cases.corrCaseExample))))
