...
class SummaryReportTableViewSpec extends ViewSpec {
  ...
  "summaryReportTable view" should {
    ...
    val reportResults: Paged[ResultGroup] = Paged(Seq(
      ...
      SimpleResultGroup(4, StatusResultField(ReportField.Status.fieldName, Some(PseudoCaseStatus.CANCELLED)), List(NumberResultField(ReportField.ElapsedDays.fieldName, Some(2)))),
      SimpleResultGroup(6, StatusResultField(ReportField.Status.fieldName, Some(PseudoCaseStatus.OPEN)), List(NumberResultField(ReportField.ElapsedDays.fieldName, Some(8)))),
      SimpleResultGroup(7, StatusResultField(ReportField.Status.fieldName, Some(PseudoCaseStatus.NEW)), List(NumberResultField(ReportField.ElapsedDays.fieldName, Some(4))))
    ))

    "render a header for each field" in {
      val doc = view(summaryReportTable(report, SearchPagination(), reportResults, Map.empty, Map.empty, "summary-report"))
