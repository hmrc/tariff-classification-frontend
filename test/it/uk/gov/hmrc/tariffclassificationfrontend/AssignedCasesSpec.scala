...
import uk.gov.hmrc.tariffclassificationfrontend.models.{CaseStatus, Operator, Pagination}
...
class AssignedCasesSpec extends IntegrationTest with MockitoSugar {
  val testCasesServiceUrl = "/cases?application_type=BTI&assignee_id=some&status=OPEN,REFERRED,SUSPENDED&sort_by=days-elapsed" +
