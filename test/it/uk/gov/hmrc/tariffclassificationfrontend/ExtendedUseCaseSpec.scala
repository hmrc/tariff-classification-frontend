...
import uk.gov.hmrc.tariffclassificationfrontend.models.{Cancellation, CaseStatus}
...
class ExtendedUseCaseSpec extends IntegrationTest with MockitoSugar {
  ...
  "Case Extended Use Change" should {
    val c = aCase(withReference("1"), withStatus(CaseStatus.CANCELLED), withDecision(cancellation = Some(Cancellation(applicationForExtendedUse = true))))
