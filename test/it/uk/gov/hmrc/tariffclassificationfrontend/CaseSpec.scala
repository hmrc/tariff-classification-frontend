...
class CaseSpec extends IntegrationTest with MockitoSugar {
  ...
  "Case activity details" should {
    ...
    "return status 200" in {
      ...
      givenAuthSuccess()
      stubFor(get(urlEqualTo(s"/events?case_reference=1&type=CASE_STATUS_CHANGE&type=APPEAL_STATUS_CHANGE&type=APPEAL_ADDED&type=EXTENDED_USE_STATUS_CHANGE&type=ASSIGNMENT_CHANGE&type=QUEUE_CHANGE&type=NOTE&page=1&page_size=${Pagination.unlimited}"))
        .willReturn(aResponse()
          .withStatus(OK)
