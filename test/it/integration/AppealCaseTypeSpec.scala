  ...
  "Case Review Change" should {
    ...
    def shouldFail = {
      ...
      val response: WSResponse =
        await(ws.url(s"http://localhost:$port/manage-tariff-classifications/cases/1/new-appeal/ANY").get())
    ...
    def shouldSucceed(): Unit = {
      ...
      AppealType.values.foreach { appealType =>
        // When
        val response: WSResponse =
          await(ws.url(s"http://localhost:$port/manage-tariff-classifications/cases/1/new-appeal/$appealType").get())
